...
class FileController extends Controller
{
    ...
    public function fileDetail($username, $id_file)
    {
        ...
        $data['file'] = File::with('user:id_user,fullname,username,pp')->where('id_file', $id_file)->where('id_file', $id_file)->whereIn(
            'id_user',
            fn (\Illuminate\Database\Query\Builder $query) =>
            $query->select('id_user')->from('users')->where('username', $username)->get()
        )->first(['original_filename', 'generate_filename', 'judul_file', 'status', 'mime_type', 'id_file', 'file_size', 'deskripsi', 'created_at', 'id_user', 'ekstensi_file', 'updated_at']);
    ...
    public function fileShareDetail($username, $id_file)
    {
        ...
        $shareFile = DB::table('files AS f')
            ...
            $query->select('id_user')->from('users')->where('username', $username)->get())
            ->first(['p.pesan', 'f.original_filename', 'f.generate_filename', 'f.judul_file', 'f.status', 'f.mime_type', 'f.ekstensi_file', 'f.id_file', 'u.fullname','u.username', 'u.pp', 'f.file_size', 'f.deskripsi', 'f.created_at', 'f.updated_at']);
